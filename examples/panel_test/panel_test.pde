...
loop ()
{
  ...
  // modified function to set a random start color and use the wheel
  lcd.clear ();
  lcd.setCursor (0, 0);
  lcd.print ("Wheel Wipe");

  strip.clear ();
  strip.bgAll (0x040404);	// set background color
  RainbowWipe (10);
  ...
  for (int xx = 0; xx < 6; xx++)
    {
      strip.clear ();
      strip.circle (5, 7, xx, 0xaf004f);
      strip.show ();
      delay (500);
    }

  // this will clear the background color and leave
  // the large circle intact then re-draw the 
  // background color
  ...
  for (int xx = 0; xx < steps; xx++)
    {
      ...
      int x2 = boxWidth - 2;
      int y2 = boxHeight - 4;
      strip.boxFill (x1 + xx, y1 + xx, x2 - xx, y2 - xx,
		     strip.Color (0, 0, 255));
      strip.show ();
      delay (500);
      strip.clear ();
  ...
  for (int xx = 0; xx < steps; xx++)
    {
      ...
      int x2 = boxWidth - 1;
      int y2 = boxHeight - 1;
      strip.box (x1 + xx, y1 + xx, x2 - xx, y2 - xx,
...
rainbowCycle (uint8_t wait)
{
  ...
  for (j = 0; j < 256 * 5; j++)
    {				// 5 cycles of all 256 colors in the wheel
      for (i = 0; i < strip.numPixels (); i++)
	{
...
// fill the dots along the string with the color wheel
// starting with a random position in the wheel
void
RainbowWipe (uint8_t wait)
{
  int i;
  int j = random (0, 255);
  for (i = 0; i < strip.numPixels (); i++)
